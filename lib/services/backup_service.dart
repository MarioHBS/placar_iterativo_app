...
class BackupService {
  ...
  Future<Map<String, dynamic>> exportComplete() async {
    try {
      ...
      // Exportar partidas
      for (final key in matchesBox.keys) {
        final match = matchesBox.get(key);
        if (match != null) {
  ...
  Future<ImportResult> importCompleteFromFile(File file,
      {bool importTournaments = true, bool importMatches = true}) async {
    try {
      final fileName = file.path.toLowerCase();
      ...
      if (fileName.endsWith('.zip')) {
        ...
        final content = await file.readAsString();
        return await importComplete(content,
            importTournaments: importTournaments, importMatches: importMatches);
      }
    } catch (e) {
      return ImportResult(
        success: false,
  ...
  Future<ImportResult> _importFromZip(File zipFile,
      {required bool teamsOnly,
      bool importTournaments = true,
      bool importMatches = true}) async {
    try {
      ...
      final imagesDir = Directory(
          '${tempDir.path}/backup_images_${DateTime.now().millisecondsSinceEpoch}');
      ...
      if (teamsOnly) {
        ...
        result = await _importCompleteWithImages(jsonContent, imageMapping,
            importTournaments: importTournaments, importMatches: importMatches);
      }
  ...
  Future<ImportResult> _importTeamsWithImages(
      String jsonContent, Map<String, String> imageMapping) async {
    try {
      final data = jsonDecode(jsonContent) as Map<String, dynamic>;
      ...
      for (final teamData in teams) {
        try {
          final team = await _teamFromJsonWithImages(
              teamData as Map<String, dynamic>, imageMapping);
          await teamsBox.put(team.id, team);
          importedCount++;
        } catch (e) {
          print('Erro ao importar time: $e');
  ...
  Future<ImportResult> _importCompleteWithImages(
      String jsonContent, Map<String, String> imageMapping,
      {bool importTournaments = true, bool importMatches = true}) async {
    try {
      final data = jsonDecode(jsonContent) as Map<String, dynamic>;
      ...
      if (importTournaments) {
        for (final tournamentData in tournaments) {
          try {
            final tournament =
                _tournamentFromJson(tournamentData as Map<String, dynamic>);
      ...
      // Importar partidas se solicitado
      if (importMatches) {
        for (final matchData in matches) {
          try {
            final match = _matchFromJson(matchData as Map<String, dynamic>);
            await matchesBox.put(match.id, match);
            matchesImported++;
          } catch (e) {
            print('Erro ao importar partida: $e');
          }
        }
      }
      ...
      return ImportResult(
        ...
        message:
            'Backup importado com sucesso! ($teamsImported times, $tournamentsImported torneios, $matchesImported partidas)',
      );
    } catch (e) {
      return ImportResult(
        success: false,
  ...
  Future<Team> _teamFromJsonWithImages(
      Map<String, dynamic> json, Map<String, String> imageMapping) async {
    String? finalImagePath;

    if (json['imagePath'] != null && json['imagePath'].toString().isNotEmpty) {
      final originalImagePath = json['imagePath'] as String;
      final imageName =
          'team_${json['id']}_${originalImagePath.split(Platform.pathSeparator).last}';
  ...
  Future<ImportResult> importComplete(
    ...
    try {
      ...
      // OPCIONAL: Importar partidas
      if (importMatches && data['matches'] != null) {
        final matchesBox = await Hive.openBox<Match>('matches');
        final matches = data['matches'] as List;

        for (final matchData in matches) {
          final match = _matchFromJson(matchData);
          await matchesBox.put(match.id, match);
          matchesImported++;
        }
      }
      ...
      if (importTournaments &&
          ...
          matchesImported > 0) {
        message =
            'Backup completo importado ($teamsImported times + $tournamentsImported torneios + $matchesImported partidas)';
      } else if (importTournaments && tournamentsImported > 0) {
        message =
            'Backup importado ($teamsImported times + $tournamentsImported torneios)';
      } else if (importMatches && matchesImported > 0) {
        message =
            'Backup importado ($teamsImported times + $matchesImported partidas)';
      } else {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        ...
        if (externalDir != null) {
          ...
          final publicDir =
              Directory('/storage/emulated/0/Download/Placar_Iterativo');
          backupDir = publicDir;
        } else {
      ...
      final teamImages = teams != null
          ? await _collectTeamImages(teams)
          : <String, Uint8List>{};

      if (teamImages.isNotEmpty) {
        // Criar arquivo ZIP com JSON e imagens
        final zipFilename = filename.replaceAll('.json', '.zip');
        final zipPath = await _createZipBackup(
            backupDir.path, zipFilename, content, teamImages);
        return zipPath;
      } else {
        // Salvar apenas o arquivo JSON
        final file =
            File('${backupDir.path}${Platform.pathSeparator}$filename');
      ...
      if (team.imagePath != null && team.imagePath!.isNotEmpty) {
        try {
          ...
          if (await file.exists()) {
            ...
            final imageName =
                'team_${team.id}_${file.path.split(Platform.pathSeparator).last}';
  ...
  Future<String> _createZipBackup(String backupDirPath, String zipFilename,
      String jsonContent, Map<String, Uint8List> images) async {
  ...
  bool _validateTournamentReferences(
      Map<String, dynamic> tournamentData, List teams) {
    final teamIds = teams.map((t) => t['id']).toSet();
    final tournamentTeamIds =
        List<String>.from(tournamentData['teamIds'] ?? []);
  ...
  Map<String, dynamic> tournamentToJson(Tournament tournament) =>
      _tournamentToJson(tournament);
  Tournament tournamentFromJson(Map<String, dynamic> json) =>
      _tournamentFromJson(json);
  Map<String, dynamic> gameConfigToJson(GameConfig config) =>
      _gameConfigToJson(config);
  GameConfig gameConfigFromJson(Map<String, dynamic> json) =>
      _gameConfigFromJson(json);

  void validateBackupFormat(String jsonContent) =>
      _validateBackupFormat(jsonContent);
  ...
  Map<String, dynamic> createCompleteBackup(
      List<Team> teams, List<Tournament> tournaments) {
    return {
      'type': 'complete',
      'version': '1.0',
      'exportDate': DateTime.now().toIso8601String(),
      'teams': teams.map((team) => _teamToJson(team)).toList(),
      'tournaments': tournaments
          .map((tournament) => _tournamentToJson(tournament))
          .toList(),
    };
  }
  ...
  // Métodos de serialização para Match
  ...
  // Métodos públicos para serialização
  Map<String, dynamic> matchToJson(Match match) => _matchToJson(match);
  Match matchFromJson(Map<String, dynamic> json) => _matchFromJson(json);
}
